...
class Users_Model extends CI_Model {
	...
	/* 
	Same as the above function, much easy to understand
	We only use this to decrypt the hashed the session username 
	
	note: camelcased. decrypt Username
	*/
	public function decryptUsername($hash)
	{
		$query = $this->db->query("SELECT username FROM ".$this->users_table." WHERE MD5(SHA1(username)) = '".$hash."'");
